...
public class IdentifierDataSourceChecker extends IdCardPoliceChecker {
    /*调用新数据源平台访问第三方服务
     * URL: http://<serveradress>:<port>/dsp/api/resource/identifier
     * IdentifierRequestModel（数据源请求Model）
     * 属性字段	类型	是否必填	描述
     * name	String	是	姓名
     * idNo	String	是	身份证号
     * 接口 输出参数
	...
	protected IdCardPoliceChecker.Result doCheck(String name, String number,String appId)
		...
		try {
			...
			String checkUrl = baseUrl +"/dsp/api/resource/identifier";
