 ...
 * @date 2017年8月10日
 */
public class CheckUserOnBuzTypeMobileThreeForCLHandler extends NonBlockingJobHandler{
	
	static final String url = StartupConfig.get("dsp.api.buzType.checkMobileThree");
	@Override
	public void execute(String appId) throws RetryRequiredException {
		if(!isOpenSwitch("BuzTypeSwitch")){
			return;
		}
		Logger.get().info("CheckUserOnBuzTypeMobileThreeForCLHandler execute appId:" + appId);
		Map<String, Object> param = getUserBaseInfoModel(appId);
		if(param==null){
			Logger.get().info("CheckUserOnBuzTypeMobileThreeForCLHandler not execute,because the dsp param is null, appId:" + appId);
			return;
		}
		try {
			//调用手机三要素dsp
            ...
            RawDataStorageManager.addRawDatas(new RawData(appId,RawDataVariableNames.BUZTYPE_MOBILETHREE,new Gson().toJson(model)));
          //写mongo
            AppDerivativeVariableManager.addVariables(new AppDerivativeVariablesBuilder(appId)
            		.isNotNullAdd(DerivativeVariableNames.MOBILETHREE_RESULT,Integer.parseInt(model.getResult()))
            		.isNotNullAdd(DerivativeVariableNames.MOBILETHREE_MESSAGE,model.getMessage())
            		.build());
