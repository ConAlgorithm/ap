...
public class A006 extends FraudRule {

	public A006(String id, String name, int score, String description) {
    ...
    AppDerivativeVariables variables = AppDerivativeVariableManager.getVariables(
        appID,
        AppDerivativeVariableNames.SECOND_CONTACT_MOBILE_PROVINCE,
        AppDerivativeVariableNames.THIRD_CONTACT_MOBILE_PROVINCE,
        AppDerivativeVariableNames.ADDITIONAL_CONTACT_MOBILE_PROVINCE_11,
        AppDerivativeVariableNames.ADDITIONAL_CONTACT_MOBILE_PROVINCE_12,
        AppDerivativeVariableNames.ADDITIONAL_CONTACT_MOBILE_PROVINCE_13);
    Set<String> citySet = new HashSet<>();

		for (Integer type : colleaguePersonTypes) {

			if (ContactPersonType.SecondContactPerson.getValue() == type) {
				citySet.add(variables.getAsString(AppDerivativeVariableNames.SECOND_CONTACT_MOBILE_PROVINCE));
			}
			else if (ContactPersonType.ThirdContactPerson.getValue() == type) {
				citySet.add(variables.getAsString(AppDerivativeVariableNames.THIRD_CONTACT_MOBILE_PROVINCE));
			}
			else if (ContactPersonType.AdditionalContactPerson_1.getValue() == type) {
				citySet.add(variables.getAsString(AppDerivativeVariableNames.ADDITIONAL_CONTACT_MOBILE_PROVINCE_11));
			}
			else if (ContactPersonType.AdditionalContactPerson_2.getValue() == type) {
				citySet.add(variables.getAsString(AppDerivativeVariableNames.ADDITIONAL_CONTACT_MOBILE_PROVINCE_12));
			}
			else if (ContactPersonType.AdditionalContactPerson_3.getValue() == type) {
			  citySet.add(variables.getAsString(AppDerivativeVariableNames.ADDITIONAL_CONTACT_MOBILE_PROVINCE_13));
			}
		}
