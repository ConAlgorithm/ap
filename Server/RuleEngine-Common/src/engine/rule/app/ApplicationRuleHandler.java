...
public abstract class ApplicationRuleHandler<T> extends RuleHandler<T> {
	...
	protected void recordReasonIfRejected(String appId, DecisionInOutForm form) {
		if (form.getDecisionResult() == RuleEngineDecisionResult.Rejected.getValue()) {
			ApplicationHelper.RecordRejectReason(appId, form.getDecisionRejectReasonSet(),
					RejectedType.RuleEngineCheckFailed);
		}
	}

	// 保存规则执行结果
	protected void ProcessRuleResult(String appId, String jobName, boolean isPassed, int score, BaseForm... outForms) {
		try {
			if (outForms == null || outForms.length == 0) {
				RuleEngineDao.saveRuleEngineResult(appId, jobName, isPassed, score, null);
				return;
			}
			...
			String execSeqStr = CommonUtil.composeConnectString(decisionForm.getRandomNumResultList(), ",");
			RuleEngineDao.saveRuleEngineResult(appId, jobName, isPassed, score, execSeqStr);
			...
			while (iter.hasNext()) {
				RuleEngineDao.saveRuleEngineResultDetail(ruleResultId, iter.next(), score);
			}
			...
			Logger.get().error(String.format("Save RuleEngine execute result of AppId: %s error", appId), e);
		}
	}

	public void buildScoreResultObj(RuleEngineScoreResultObject obj, BaseForm outForm) {
		if (outForm == null)
			return;
		...
		} else if (outForm instanceof StoreInOutForm) {
			StoreInOutForm storeForm = (StoreInOutForm) outForm;
			obj.StoreLevel = storeForm.getStoreLevelResult();
		}
	}
	...
	public void recordReasonAndCount(String appId, String reason, int count) {
		AppDerivativeVariableManager.addVariables(
				new AppDerivativeVariable(appId, AppDerivativeVariableConsts.ReuploadReason, reason),
				new AppDerivativeVariable(appId, AppDerivativeVariableConsts.ReuploadRestCount, count));
	}
}
