...
public class BlackCheckRuleHandler extends ApplicationRuleHandler<QueueMessager>{
	...
	public QueueMessager handle(QueueMessager message) {
		...
		if (!DatabaseApiUtil.isNormalOrTestUser(appId)) {
			Logger.get().debug(String.format("The user of AppId: %s is not a normal or test user.", appId));
			message.setJobStatus(JobStatus.Approved);
			return message;
		}
