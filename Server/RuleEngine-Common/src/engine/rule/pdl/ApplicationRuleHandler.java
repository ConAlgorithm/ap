...
public abstract class ApplicationRuleHandler<T> extends RuleHandler<T> {
	...
	protected boolean isForTesting = StartupConfig.getAsBoolean("catfish.ruleengine.isForTestingPDL");
	...
	protected void recordReasonIfRejected(String appId, DecisionInOutForm form)
	{
		...
		{
			recordReasonIfRejected(appId, form.getDecisionRejectReasonSet());
		}
	}
	
	protected void recordReasonIfRejected(String appId, Set<String> rejectReasonSet)
	{
		ApplicationHelper.RecordRejectReason(appId, rejectReasonSet, RejectedType.RuleEngineCheckFailed);
	}
