...
public class PDLRepaymentSuccessDefinition implements MessageDefinition {
	...
	public List<Message> apply(Map<String, ?> receivedMessage) {
		...
		String shortMessageContent = String.format(ResourceConfig.getResourceByName(
				...
				model.getBill().getBillStartDate().getMonth() + 1, model.getPlanAmount().getTotalAmount(), 
				model.getPlanAmount().getWithholdResult().getWithholdAmount());
