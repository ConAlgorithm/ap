...
public class PDLAccountantReminderDefinition implements MessageDefinition {
	...
	public List<Message> apply(Map<String, ?> receivedMessage) {
		...
		String shortMessageContent = String.format(ResourceConfig.getResourceByName(
				...
				model.getBill().getBillStartDate().getMonth() + 1, model.getBill().getTotalAmount(), model.getBill().getMinAmount(), 
				model.getBill().getBillDueDate().getMonth() + 1, model.getBill().getBillDueDate().getDate());
